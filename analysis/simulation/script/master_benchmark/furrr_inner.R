...
run_furrr_inner <- function(){
  # Process each scenario
  for (i in seq_along(scenarios)) {
    scenario <- scenarios[[i]]

    # Simulations
    simulations <- future_map(1:n_simulations, function(j) {
      sim <- simulate_groups(
        duration = 100,
      ...
      sim$simulation <- j
      return(sim)
    }, .env_globals = list(scenario = scenario), .options = furrr_options(seed = NULL))

    # Results
    results <- future_map(.x = simulations, ~process_simulation(sim = .x, peak_coeffs, scenario),
                          .env_globals = list(peak_coeffs = peak_coeffs, scenario = scenario),
