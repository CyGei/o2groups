...
# STATS ---------------------------------------------------------------------
library(tidyverse)
pal = c("A" = "magenta", "B" = "orange", "C" = "forestgreen")
gg_col <- list(
...
p_total_Ri <- ggplot(total_Ri_sims,
  ...
  geom_hline(aes(yintercept = 3), col = "#4C4E52", lty = "solid")+
  facet_grid(~ group) +
  theme_bw()+
...
Truth <- generate_Mcol( n_groups = 3,
               size = c(10000, 10000, 10000),
               name = c("A", "B", "C"),
               delta = c(10, 2, 1)) %>%
  as.data.frame(.) %>%
...
ggplot(mix_t) +
  ...
  geom_segment(data = Mcol_df, aes(x = 0, xend = 20, y = freq, yend = freq),
               size = 1, col = "black")+
  geom_point(data = Mcol_df, aes(median(0:20), y = freq),
             size = 2, col = "black")+
...
#early phase
Mcol <- get_mixing(data, min_t = 0, max_t = 20)$Mcol

Mcol_to_delta(n_groups = 3,
              size = c(10000, 10000, 10000),
              name = c("A", "B", "C"),
              Mcol = Mcol)
...
#Plot optim
timepoints <- 1:100
output <- matrix(NA, nrow = length(timepoints), ncol = 3)
...
Mt40 <- Ri_means %>%
  filter(date_infection == 40) %>%
  select(-date_infection) %>%
  pivot_wider(names_from = group, values_from = Ri) %>%
